...
sub new {
    ...
    my $shard = $args->{datasource};
    croak "Datasource '" . $shard->name . "' does not appear to be a shard"
      if not ref $shard eq 'DBIx::Router::DataSource::Group::shard';
    ...
    $args->{match} = [
        ...
        },
        {
            structure => 'columns',
            operator  => 'all',
            tokens    => [ $shard->table . '.' . $shard->column ],
        },
    ];
