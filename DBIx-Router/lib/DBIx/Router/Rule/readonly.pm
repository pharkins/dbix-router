package DBIx::Router::Rule::readonly;

use warnings;
use strict;

use base qw(DBIx::Router::Rule::regex);
use Carp;
...
sub new {
    ...
    $args->{match}     = $self->_compile_regexes( ['^ \s* SELECT \b '] );
    $args->{not_match} = $self->_compile_regexes( ['\b FOR \s+ UPDATE \b '] );
