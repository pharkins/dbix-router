...
sub new {
    ...
    my $rule_args = delete $args->{rule};
    my $class     = delete $rule_args->{class};
    if ( $class !~ m/::/ ) {
        $class = 'DBIx::Router::Rule::' . $class;
    }
    DBIx::Router->_load_class($class)
      or croak("Failed to load rule class '$class': $@");

    my $rule = $class->new( { %{$rule_args}, datasource => 'dummy' } );
    $args->{rule} = $rule;
...
sub accept {
    my ( $self, $request ) = @_;
    return not $self->rule->accept($request);
}
